...
const HallRow = ({ hall, reverse, activeIndex, onChangeIndex, onBook, ctaAlign = 'center' }: HallRowProps) => {
  ...
  useEffect(() => {
    if (rowInView.inView && !activated) {
      setActivated(true)
    }
  }, [rowInView.inView, activated])

  const visible = activated || rowInView.inView

  return (
    <div
      className={cn(
        styles.hallRow,
        reverse && styles.hallRowReverse,
        styles.hallCardWrapper,
        visible && styles.hallCardWrapperVisible
      )}
      ref={rowInView.ref}
    >
      <HallCard
        ...
        onBookHall={(context) => onBook(context)}
      />
    </div>
  )
}
...
export const BanquetsPage = ({ onNavigate, onBook }: BanquetsPageProps) => {
  ...
  const textInView = useInView<HTMLDivElement>({ threshold: 0.2 })
  ...
  return (
    ...
    <section className={styles.mediaRowSection}>
      ...
      <section className={styles.textSection}>
        <div
          className={cn(styles.textContent, textInView.inView && styles.textContentVisible)}
          ref={textInView.ref}
        >
          {FAMILY_TEXT.map((paragraph, index) => (
            <p key={index} className="typo-highlight">
              {paragraph}
            </p>
          ))}
        </div>
