...
const HALLS = [
  {
    ...
    media: [menu1, menu2, menu3, menu4, menu5],
  },
  {
    id: 'upper-vip',
    name: 'Верхний вип',
    ...
    media: [menu6, menu7, menu8, menu5, menu4],
  },
  {
    id: 'lower-vip',
    name: 'Нижний вип',
    ...
    media: [menu3, menu2, menu1, menu7, menu8],
  },
  {
    id: 'terrace',
    name: 'Летняя веранда',
    ...
    media: [menu4, menu5, menu6, menu2, menu3],
  },
] as const
        ...
        <div className={styles.hallWrapper}>
          ...
          <div className={styles.hallCard}>
            <div className={styles.infoColumn}>
              <h3 className={styles.hallName}>{activeHall.name}</h3>
              <p className={styles.description}>{activeHall.descriptionPrimary}</p>
              <p className={styles.description}>{activeHall.descriptionSecondary}</p>
            ...
            <div className={styles.galleryColumn}>
              <figure className={styles.mainMedia}>
                <img src={activeImage} alt={activeHall.name} />
              </figure>
              <div className={styles.thumbRow}>
                {activeHall.media.map((mediaSrc, index) => (
                  <button
                    key={`${activeHall.id}-${index}`}
                    className={styles.thumb}
                    onClick={() => handleThumbClick(index)}
                    aria-label={`Показать фото ${index + 1}`}
                  >
                    <img src={mediaSrc} alt="" />
                    {index === activeMediaIndex && <span className={styles.thumbActiveBorder} />}
                  </button>
                ))}
              </div>
            </div>
