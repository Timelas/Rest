...
const KIDS_FEATURE_COPY = `Дети в Satrapezo всегда самые важные гости. Мы готовим для них отдельное меню, мастер-классы и продуманную анимацию, чтобы родителям было спокойно, а детям — весело.`
...
const FINAL_GALLERY = [
  ...
  { id: 'kg-8', src: PLACEHOLDER_BURGUNDY, alt: 'Детская галерея 8' },
] as const
...
export const KidsAnimationPage = ({ onNavigate, onBook }: KidsAnimationPageProps) => {
  ...
  return (
    <div className={styles.page}>
      ...
      <section className={styles.section} style={{ marginTop: '126px' }}>
        ...
        <div className={styles.kidsSliderWrapper}>
          <div className={styles.kidsSliderTrack} ref={trackRef}>
            {duplicatedSlides.map((item, index) => (
              ...
              >
                <span className="typo-description-primary">{item.title}</span>
              </article>
            ))}
          </div>
        </div>
      ...
      <section className={styles.section} style={{ marginTop: '100px' }}>
        <p className={`${styles.centerText} typo-description-secondary`}>{TEXT_BLOCKS[0]}</p>
      </section>
      ...
      <section className={styles.section} style={{ marginTop: '100px' }}>
        <div className={styles.textBlock}>
          {TEXT_BLOCKS.map((paragraph, idx) => (
            <p key={idx} className="typo-description-secondary">
              {paragraph}
            </p>
          ))}
        </div>
