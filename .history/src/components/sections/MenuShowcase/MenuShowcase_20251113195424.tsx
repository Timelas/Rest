...
export const MenuShowcase = () => {
  const showcase = useInView<HTMLDivElement>({ threshold: 0.4 })

  return (
    <div className={styles.section} ref={showcase.ref}>
      <div className={styles.inner}>
        <h2 className={cn(styles.title, 'typo-h2', showcase.inView && styles.titleVisible)}>Меню</h2>
        <div className={styles.grid}>
          {MENU_ITEMS.map((item, index) => (
            <article
              key={item.id}
              className={cn(styles.card, showcase.inView && styles.cardVisible)}
